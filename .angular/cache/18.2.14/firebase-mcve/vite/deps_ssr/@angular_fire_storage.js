...
var FetchConnection = class {
  constructor() {
    this.errorText_ = "";
    this.sent_ = false;
    this.fetch_ = import_undici.fetch;
  ...
  send(url, method, body, headers) {
    return __async(this, null, function* () {
      ...
      this.sent_ = true;
      try {
        const response = yield this.fetch_(url, {
          method,
          headers: headers || {},
          body
        });
        ...
        this.errorCode_ = ErrorCode.NO_ERROR;
        this.body_ = yield response.arrayBuffer();
      } catch (e) {
...
var FetchBytesConnection = class extends FetchConnection {
  getResponse() {
    if (!this.body_) {
...
var FetchStreamConnection = class extends FetchConnection {
  constructor() {
    super(...arguments);
    this.stream_ = null;
  }
...
function getStreamInternal(ref3, maxDownloadSizeBytes) {
  ref3._throwIfRoot("getStream");
  const requestInfo = getBytes$1(ref3.storage, ref3._location, maxDownloadSizeBytes);
  ...
  const result = maxDownloadSizeBytes !== void 0 ? new TransformStream(newMaxSizeTransform(maxDownloadSizeBytes)) : new TransformStream();
  ref3.storage.makeRequestWithTokens(requestInfo, newStreamConnection).then((readableStream) => readableStream.pipeThrough(result)).catch((err) => result.writable.abort(err));
...
function getStream(ref3, maxDownloadSizeBytes) {
  ref3 = getModularInstance(ref3);
  return getStreamInternal(ref3, maxDownloadSizeBytes);
}
